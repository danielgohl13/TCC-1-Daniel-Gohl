...
Um \textit{framework} baseado em uma \textit{Convolutional Neural Network} - CNN\todo{Traduzir} foi proposto por \citeonline{Qin:2016} para reconhecimento de peixes em vídeos gravados submersos em água\todo{Qual tipo? Mar, rio?}. No trabalho as imagens de fundo são removidas utilizando métodos baseados em decomposição de matrizes esparsas e de baixo nível, removendo assim apenas o fundo da imagem, deixando somente o peixe.

Os dados extraídos são passados para camadas de convolução depois para uma camada não linear, em seguida é feita a classificação utilizando SVM (Support Vector Machine)\todo{Adicionar referência}. A taxa de aprendizado diminui a medida que a quantidade de parâmetros são adicionados, necessitando assim de uma otimização no filtro de aprendizado. 
...
\todo[inline]{Seria ideal explicar a \autoref{fig:trainingchuan}}
...
\todo[inline]{Descrever como o método de \citeonline{tang1998automatic} pode contribuir com o seu trabalho}
